...
export function AgentInterceptModal() {
  ...
  useEffect(() => {
    const notification = notifications.find(
      (n) => (n.type as string) === "agent_approval",
    );
    if (notification && notification.metadata) {
      setActiveApproval(
        notification.metadata as unknown as {
          approval_id: string;
          payload_hash: string;
        },
      );
  ...
  const signPayload = async (hash: string) => {
    ...
    const rawKey = window.crypto.subtle.importKey(
      "raw",
      encoder.encode(
        localStorage.getItem("user_session_key") || "temp-secret-key-123",
      ),
      ...
      <DialogContent className="max-w-2xl bg-zinc-950 border border-zinc-800 text-zinc-100">
        <DialogHeader>
          ...
          <DialogDescription className="text-zinc-400">
            A machine agent is attempting to mutate vault state. Review the
            cryptographic diff before signing off.
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="mt-4 space-y-4">
            <div className="p-4 bg-zinc-900 border border-zinc-800 rounded font-mono text-xs overflow-x-auto text-zinc-300">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <h4 className="text-zinc-500 mb-2 uppercase tracking-wide text-[10px] font-bold">
                    Old State
                  </h4>
            ...
            <div className="flex items-center justify-between text-xs text-zinc-500 font-mono">
              <span
                title={activeApproval.payload_hash}
