...
Accepts or rejects an interactive CLI workflow generated by a Human-in-The-Loop required action.
...
**Interactive Visual Diff:**
Before finalizing the approval, the CLI now prints a visual representation of how the environment will change.
- `+` (Green): Additions.
- `-` (Red): Deletions.
- `~` (Yellow): Modifications.

You will be prompted to finalize the execution `(y/N)`.

**Non-Interactive Execution:**
If executing the `approve` command in a CI/CD pipeline or non-TTY environment, the command **will immediately fail** to prevent blind approvals unless explicitly overridden. This is a critical security feature designed to prevent supply-chain attacks that blindly execute CLI tokens.
## `env`
